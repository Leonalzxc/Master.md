...
export async function selectWorker(jobId: string, bidId: string, locale: string) {
  ...
  // The RPC validates ownership/status, derives the worker from the bid, and updates atomically.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: selectedWorkerId, error } = await (supabase.rpc as any)('select_worker_for_job', {
    p_job_id: jobId,
    p_bid_id: bidId,
  });
  if (error) throw new Error(error.message);
  if (!selectedWorkerId) throw new Error('Worker selection failed');
